...
import { getOrCreateSessionKey, generateIdempotencyKey } from "./client-manager";
import { stripThinkingBlocks } from "./markdown-utils";
import type { Preferences, DisplayMessage, ChatEventPayload, ChatMessage } from "./types";
import os from "os";
...
function parseHistoryEntry(entry: unknown): DisplayMessage | null {
    ...
    return {
        id: (e.id as string) ?? `hist-${Date.now()}-${Math.random().toString(36).slice(2, 6)}`,
...
export default function ChatCommand() {
    ...
    const connect = useCallback(async () => {
        ...
        try {
            const client = new GatewayClient({
                ...
                onStateChange: (state) => {
                    ...
                    if (state === "connected") {
                        ...
                        showToast({ style: Toast.Style.Animated, title: "重连中..." });
                ...
                onError: (err) => {
                    ...
                    setConnPhase(msg.includes("pairing") || msg.includes("not paired") || msg.includes("pending") ? "pairing" : "error");
                    setConnError(msg);
                    showToast({ style: Toast.Style.Failure, title: "连接错误", message: msg });
                },
            });
            ...
            client.on("chat", (payload) => {
                ...
                if (evt.state === "delta") {
                    if (streamingRunIdRef.current && streamingRunIdRef.current !== evt.runId) return;
                    ...
                    const finalContent = stripThinkingBlocks(extractTextContent(evt.message));
                    const completeContent = finalContent || stripThinkingBlocks(streamingAccumulator.current);
                    ...
                    if (completeContent) {
                        setMessages((prev) => {
                            ...
                            if (last?.role === "assistant" && last.content === completeContent) return prev;
                            return [...prev, { id: finalId, role: "assistant" as const, content: completeContent, timestamp: Date.now() }];
                        });
                        setSelectedId(finalId);
                    }
                    // 延迟关闭 streaming，确保选中先切换到 final
                    setTimeout(() => { if (isMounted.current) setIsStreaming(false); }, 0);
                    ...
                    streamingRunIdRef.current = "";
                    showToast({ style: Toast.Style.Failure, title: "AI 响应错误", message: evt.errorMessage ?? "未知错误" });
            ...
            // ── 加载历史消息 ──
            try {
                const histRaw = await client.request<unknown>("chat.history", { sessionKey: sessionKeyRef.current, limit: 50 });
                ...
                if (entries.length > 0) {
                    const parsed = entries.map(parseHistoryEntry).filter((m): m is DisplayMessage => m !== null);
                    if (isMounted.current && parsed.length > 0) {
                        ...
                        showToast({ style: Toast.Style.Success, title: `已加载 ${sorted.length} 条历史消息` });
                    }
                }
            } catch (err) {
                showToast({ style: Toast.Style.Failure, title: "历史加载失败", message: err instanceof Error ? err.message : String(err) });
            }
            ...
            setConnPhase(msg.includes("pairing") || msg.includes("not paired") || msg.includes("pending") ? "pairing" : "error");
            setConnError(msg);
            showToast({ style: Toast.Style.Failure, title: "连接失败", message: msg });
        }
    }, []);
    ...
    useEffect(() => {
        ...
        connect();
        return () => { isMounted.current = false; clientRef.current?.disconnect(); clientRef.current = null; };
    }, [connect]);
    ...
    const sendMessage = useCallback(async (text: string) => {
        ...
        setMessages((prev) => [...prev, { id: newId, role: "user" as const, content: text.trim(), timestamp: Date.now() }]);
        setSelectedId(newId);
        setSearchText("");
        ...
        try {
            ...
            showToast({ style: Toast.Style.Failure, title: "发送失败", message: err instanceof Error ? err.message : String(err) });
        }
    }, []);
    ...
    const resetSession = useCallback(async () => {
        ...
        try {
            await clientRef.current.request("sessions.reset", { key: sessionKeyRef.current });
            setMessages([]);
            setIsStreaming(false);
            setSelectedId(undefined);
            showToast({ style: Toast.Style.Success, title: "会话已重置" });
        } catch (err) {
            showToast({ style: Toast.Style.Failure, title: "重置失败", message: err instanceof Error ? err.message : String(err) });
        }
    }, []);
    ...
    const abortGeneration = useCallback(async () => {
        ...
        try {
            await clientRef.current.request("chat.abort", { sessionKey: sessionKeyRef.current, runId: streamingRunIdRef.current });
        } catch { /* 忽略 */ }
    }, []);
    ...
    if (connPhase === "pairing") {
        ...
        return (
            <Detail
                ...
                actions={<ActionPanel><Action title="重试" icon={Icon.ArrowClockwise} onAction={connect} /></ActionPanel>}
            />
        );
    }
    ...
    if (connPhase === "error") {
        return (
            <Detail
                markdown={["# ❌ 连接失败", "", `> ${connError}`, "", "请检查配置。"].join("\n")}
                actions={<ActionPanel><Action title="重试" icon={Icon.ArrowClockwise} onAction={connect} /></ActionPanel>}
            />
        );
    }
    ...
    return (
        <List
            ...
            actions={
                <ActionPanel>
                    {searchText.trim() && <Action title="发送消息" icon={Icon.ArrowRight} onAction={() => sendMessage(searchText)} />}
                    {isStreaming && <Action title="停止生成" icon={Icon.Stop} shortcut={{ modifiers: ["cmd"], key: "." }} onAction={abortGeneration} />}
                    <Action title="重置会话" icon={Icon.Trash} shortcut={{ modifiers: ["cmd", "shift"], key: "r" }} onAction={resetSession} />
                </ActionPanel>
            }
        >
            {messages.length === 0 && !isStreaming && (
                <List.EmptyView
                    title="OpenClaw Chat"
                    description={connPhase === "connected" ? "在上方输入消息开始对话" : "正在连接..."}
                    icon={Icon.Message}
                />
            )}
